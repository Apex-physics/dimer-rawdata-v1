...
chi_display = " , ".join([str(c) for c in sorted(available_chis_for_info)])
col_chi.metric("包含的 χ", chi_display)
...
with col_main:
    ...
    with tab_single:
        ...
        if len(times) > 0 and target_metrics:
            ...
            ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2),
                      ncol=min(3, len(target_metrics)), fontsize='small')
            # 必须配合这行，否则图例会被切掉
            fig.tight_layout()
            st.pyplot(fig)
            ...
            col_dl1.download_button("下载图片 (PNG)", buf.getvalue(), "plot.png", "image/png")

            # --- 下载 CSV ---
            # 假设你当前绘图的数据是 t_plot 和 y_plot
            df_download = pd.DataFrame({"Time": t_plot, "Value": y_plot})
            csv = df_download.to_csv(index=False).encode('utf-8')
            col_dl2.download_button("下载数据 (CSV)", csv, "data.csv", "text/csv")
    ...
    with tab_compare:
        ...
        if not (s_L and s_Init and s_Freq and s_U and s_J and s_eta):
            df_c = df_c.iloc[0:0]
        ...
        if st.session_state.compare_lines:
            ...
            # --- 多组对比图例 (右侧) ---
            ax_c.legend(loc='upper left', bbox_to_anchor=(1.02, 1),
